...
rgfi %>% 
  filter(mb %in% c(3536386, 1968823)) %>% 
  ggplot(aes(year, assets_c, group = mb)) +
  geom_point() +
  geom_line()
...
df_joined %>% count(Pravno_ustrojbeni_oblik_naziv, sort = T)

df_joined_na <- df_joined %>%
  #filter(year == "2023") %>% 
  map_dfr(~ sum(is.na(.))) %>% 
  pivot_longer(everything()) %>% 
  mutate(perc_na = round(value / 558610 * 100, 2)) %>% 
  arrange(perc_na)
...
perc_NA <- rgfi_clean %>%
  ...
  map_dfr(~ sum(is.na(.))) %>% 
  pivot_longer(everything()) %>% 
  mutate(perc_na = round(value / 595774 * 100, 2)) %>% 
  arrange(perc_na)
...
df_2023_na <- df_2023 %>%
  ...
  map_dfr(~ sum(is.na(.))) %>% 
  pivot_longer(everything()) %>% 
  mutate(perc_na = round(value / 88316 * 100, 2)) %>% 
  arrange(perc_na)

write_parquet(rps, "work/rps.parquet")
write_parquet(rgfi, "work/rgfi.parquet")
write_csv(df_2023, "work/rgfi_2023.csv")
#---------------------------------------
rgfi_2023 <- read_csv2("work/RGFI_javna_objava_2023_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2022 <- read_csv2("work/RGFI_javna_objava_2022_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2021 <- read_csv2("work/RGFI_javna_objava_2021_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2020 <- read_csv2("work/RGFI_javna_objava_2020_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2019 <- read_csv2("work/RGFI_javna_objava_2019_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2018 <- read_csv2("work/RGFI_javna_objava_2018_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2017 <- read_csv2("work/RGFI_javna_objava_2017_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2016 <- read_csv2("work/RGFI_javna_objava_2016_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2015 <- read_csv2("work/RGFI_javna_objava_2015_new.csv") %>%
  filter(vr_izvj == 10, 
...
rgfi_2014 <- read_csv2("work/RGFI_javna_objava_2014_new.csv") %>%
  filter(vr_izvj == 10, 
